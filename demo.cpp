...
{   
    ...
    GPIO_Init();
    int mode = 0,y_err=0,x_err=0,square_err=0;
     ...
     while (1)
    {
        ...
        for (int i = 0; i < 1/*boxes.size()*/; i++) {
            ...
            */
            char text[256];
            // 格式化检测结果的文本
            sprintf(text, "%s %.1f%%", class_names[boxes[0].cate], boxes[0].score * 100);
            ...
            int x = boxes[0].x1;
            int y = boxes[0].y1 - label_size.height - baseLine;
            if (y < 0)
                y = 0;
            ...
            // 在图像上画出检测框
            cv::rectangle (cvImg, cv::Point(boxes[0].x1, boxes[0].y1), 
                           cv::Point(boxes[0].x2, boxes[0].y2), cv::Scalar(255, 255, 0), 2, 2, 0);
...
void GPIO_Init(void)
{
    ...
    digitalWrite(PWM_Right_motor,LOW);
    digitalWrite(PWM_Left_motor,LOW);
}
